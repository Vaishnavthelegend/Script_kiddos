...
chrome.storage.sync.get('fallacySettings', function(results){
  if(!results['fallacySettings']){
    ...
    showFb = true;
  }else if(results['fallacySettings']['facebook'] == undefined){
    // default to showing if FB is not set
    showFb = true;
  }else{
    showFb = results['fallacySettings']['facebook']
  }
  if(showFb){
    runFallacy();
  }
...
function runFallacy(){
  ...
  function run(sourceHash){
    ...
    var sources = document.querySelectorAll(linkTextClass + ':not(.fallacy)');
    // add fallacy class to each element to make sure that script isn't run multiple times on the same element
    sources.forEach(function(e){ e.classList.add('fallacy') });
    ...
    // var commentSources = document.querySelectorAll(commentSourceClass + ':not(.fallacy)');
    // commentSources.forEach(function(e){ e.classList.add('fallacy') });
