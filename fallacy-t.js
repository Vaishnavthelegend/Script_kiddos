// set showThreads as a fail-safe in case storage.sync is not set
var showThreads = true

chrome.storage.sync.get('fallacySettings', function(results){
  if(!results['fallacySettings']){
    // default to showing if there are no saved settings
    showThreads = true;
  }else if(results['fallacySettings']['threads'] == undefined){
    // default to showing if Threads is not set
    showThreads = true;
  }else{
    showThreads = results['fallacySettings']['threads']
  }
  if(showThreads){
    runFallacy();
  }
...
// only run if showThreads

function runFallacy(){

  // function to add decal to target element
  function updateHTML(el, sourceHash){
    // NOTE: Because Threads obscures full links, can't dig for special cases like Borowitz Report
    ...
    // Do not include "opinion" parsing for now (Threads doesn't include full URLs, so there's no way to parse)
    // var opRegex = /opinion/;
    ...
    // if(opinion != null){
    //   opHTML = "<span class='fallacy-txt' style='font-size: 75%; white-space: nowrap; background-color: darkgray; color: white; border-radius: 5px'>&nbsp;OpEd&nbsp;</span>";
    // }else{
    //   opHTML = " ";
    // }
    ...
    if(sourceData == null){
      ...
      // create acc HTML
      decalAccSpan.classList.add("fallacy-txt");
      ...
      // create bias HTML
      decalBiasSpan.classList.add("fallacy-txt");
      ...
      decalLink.setAttribute('class', 'fallacy-tag');
      ...
      el[0].parentElement.parentElement.insertAdjacentElement('beforebegin', decalContainer);
      return true;
    }
  }
  ...
  function run(sourceHash){
    var linkContainer = ".x1j9u4d2"
    var linkSelector = '.x1lliihq.x193iq5w.x6ikm8r.x10wlt62.xlyipyv.xuxw1ft'
    // get individual tweets
    var sourcesArray = Array.from(document.querySelectorAll(linkContainer)).map(function(e) { return e.querySelector(linkSelector + ':not(.fallacy)') })

    // add fallacy class to each element to make sure that script isn't run multiple times on the same element
    sourcesArray.forEach(function(e){ e.classList.add('fallacy') });
  ...
  function initObserver(){
    ...
    var targetNode = document.querySelectorAll('.xb57i2i.x1q594ok.x5lxg6s.x1ja2u2z.x1pq812k.x1rohswg.xfk6m8.x1yqm8si.xjx87ck.x1l7klhg.xs83m0k.x2lwn1j.xx8ngbg.xwo3gff.x1oyok0e.x1odjw0f.x1n2onr6.xq1qtft.xz401s1.x195bbgf.x3ir7cq.x1l19134.x1m0igow.x13lgxp2.x168nmei.xu3j5b3.x1q0q8m5.x26u7qi.x1ejq31n.xt8cgyo.xj515ic.x1co6499.x2j4hbs.x1ma7e2m.x9f619.x78zum5.xdt5ytf.x1iyjqo2.x6ikm8r.xy5w88m.xh8yej3.xbwb3hm.x1x3vcui.x19xvnzb.x1y4ox9t.xev1tu8.xpr2fh2.xgzc8be.xexx8yu');
