...
chrome.storage.sync.get('fallacySettings', function(results){
  if(!results['fallacySettings']){
    ...
    showGoogle = true;
  }else if(results['fallacySettings']['google'] == undefined){
    // default to showing if Google is not set
    showGoogle = true;
  }else{
    showGoogle = results['fallacySettings']['google']
  }
  if(showGoogle){
    runFallacy();
  }
...
function runFallacy(){
  ...
  function run(sourceHash){
    ...
    if(tabCheck.length == 0){
      ...
      // vidCards.forEach(function(e){ e.classList.add('fallacy') });
      // quotCards.forEach(function(e){ e.classList.add('fallacy') });
