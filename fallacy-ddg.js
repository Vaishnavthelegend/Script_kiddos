...
chrome.storage.sync.get('fallacySettings', function(results){
  if(!results['fallacySettings']){
    ...
    showDDG = true;
  }else if(results['fallacySettings']['ddg'] == undefined){
    // default to showing if DDG is not set
    showDDG = true;
  }else{
    showDDG = results['fallacySettings']['ddg']
  }
  if(showDDG){
    runFallacy();
  }
...
function runFallacy(){
  ...
  function updateHTML(el, sourceHash){
    ...
    }else if(el[2].match(/huffpost.com\/highline/)){
    ...
    // create opinion HTML
    if(opinion != null){
      decalOpSpan.classList.add('fallacy-txt');
  ...
  function run(sourceHash){
    ...
    var j = 0;
    if(tab.length == 0){
      console.log("Not a Fallacy-eligible page");
      return true;
      ...
      // get link elements
      var standard = document.querySelectorAll(linkClass + ":not(.fallacy)");
      standard.forEach(function(e){ e.classList.add('fallacy') });
      var cards = document.querySelectorAll(cardClass + ":not(.fallacy)");
      ...
      var carouselContainer = document.querySelector('.js-ia-modules:not(.fallacy)')
      ...
      if(carouselContainer){
        ...
        var carouselModule = carouselContainer.querySelector('.module--carousel-news:not(.fallacy)');
        if(carouselModule){
          carouselModule.style.height = String(carouselModule.offsetHeight + 20) + "px";
          var carouselCardContainer = carouselModule.querySelector('.js-carousel-module-items:not(.fallacy)');
          if(carouselCardContainer){
            ...
            if(carouselCards.length > 0){
              ...
              cards.forEach(function(e){ e.classList.add('fallacy') });
              carouselContainer.classList.add('fallacy');
              carouselModule.classList.add('fallacy');
              carouselCardContainer.classList.add('fallacy');
              // isolate link activation to only the actual link span
              carouselCards.forEach(function(cCard){
                cCard.classList.add('fallacy');
      ...
      if(organicModuleContainer){
        ...
        var organicModule = organicModuleContainer.querySelector('.module--carousel-news:not(.fallacy)');
        if(organicModule){
          organicModule.style.height = String(organicModule.offsetHeight + 20) + "px";
          var organicCardContainer = organicModule.querySelector('.js-carousel-module-items:not(.fallacy)');
          if(organicCardContainer){
            ...
            if(organicCards.length > 0){
              // add fallacy class to each element to make sure that script isn't run multiple times on the same element
              cards.forEach(function(e){ e.classList.add('fallacy') });
              organicModuleContainer.classList.add('fallacy');
              organicModule.classList.add('fallacy');
              organicCardContainer.classList.add('fallacy');
              // isolate link activation to only the actual link span
              organicCards.forEach(function(oCard){
                oCard.classList.add('fallacy');
      ...
      // get link elements
      var news = document.querySelectorAll(linkClass + ":not(.fallacy)");
      news.forEach(function(e){ e.classList.add('fallacy') });
      ...
      // get link elements
      var vids = document.querySelectorAll(linkClass + ":not(.fallacy)");
      vids.forEach(function(e){ e.classList.add('fallacy') });
