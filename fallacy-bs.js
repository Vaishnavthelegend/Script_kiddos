...
chrome.storage.sync.get('fallacySettings', function(results){
  if(!results['fallacySettings']){
    ...
    showBluesky = true;
  }else if(results['fallacySettings']['bluesky'] == undefined){
    // default to showing if Bluesky is not set
    showBluesky = true;
  }else{
    showBluesky = results['fallacySettings']['bluesky']
  }
  if(showBluesky){
    runFallacy();
  }
...
function runFallacy(){
  ...
  function run(sourceHash){
    ...
    var sourcesArray = Array.from(document.querySelectorAll(linkSelector + ':not(.fallacy)'))
