...
function saveOptions(){
	// identify toggles
	var formElements = document.getElementById("fallacy-options").elements;
	...
	var fallacySettings = {}
	// populate settings
	targets.forEach(function(e){ fallacySettings[e['name']] = e['checked'] })
	chrome.storage.sync.set({
		fallacySettings: fallacySettings
	}, function(){ 
		console.log("Fallacy settings updated")
	});
}

// Restore radio buttons using preferences stored in chrome.storage
function restoreOptions(){
	var els = document.getElementById("fallacy-options").elements;
	...
	chrome.storage.sync.get('fallacySettings', function(results){
		...
		if(!results['fallacySettings']){
			...
			keys.forEach(function(key){
				if(results['fallacySettings'][key] == undefined){
					// default setting for any site should be true
					settings[key] = true;
					resubmit = true;
				}else{
					settings[key] = results['fallacySettings'][key]
				}
			});
		}
